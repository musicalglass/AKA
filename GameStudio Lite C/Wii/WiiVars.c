...
action update_Earth()
{
	...
	while ( who_wii_R ) 
	{
		// use Nunchuck Joystick
//		my.y = -buffer.stick1.joy_x / 4 ; 
//		my.z = buffer.stick1.joy_y / 5 ; 


		// Use Balance Board
//		my.y = ( buffer.board.topleft + buffer.board.botleft ) / 2 - ( buffer.board.topright + buffer.board.botright ) / 2 ; 
//		my.z = ( buffer.board.topright + buffer.board.topleft ) / 2 -  buffer.board.botright + buffer.board.botleft ) / 2 ; 
		
		// Use Infrared
		infrared_X = buffer.ir[0].ir_x ; 
		infrared_Y = buffer.ir[0].ir_y ; 
//		infrared_Size = buffer.ir[0].ir_size ; // Doesn't seem to work. Anybody know what I'm doing wrong?
