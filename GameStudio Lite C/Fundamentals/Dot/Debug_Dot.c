...
{
	layer = 3;
  	digits( 10,10,3,*,1, dot_panel.pos_x ); 
  	digits( 10,20,3,*,1, dot_panel.pos_y ); 
  	flags = VISIBLE ;
}
...
{
	if ( key_cuu ) dot_panel.pos_y-- ;
   if ( key_cud ) dot_panel.pos_y++ ;

   if ( key_cul ) dot_panel.pos_x-- ;
   if ( key_cur ) dot_panel.pos_x++ ;
}
