...
BMAP* counterBmp = "counter.bmp"; 						// loads a bitmap (the image with the eggcarton and the splattered egg) for later use
...
PANEL* gameOverPanel =											// defines the gameOver panel used at the end of the game:
{
	bmap = gameOverBmp; 											// defines the gameOver bitmap as the background image
  	digits(220,130,3,counterfont,1,eggsShipped); 		// displays the eggsShipped variable as the final score
...
{
	bmap = gameStartBmp; 
	button(300,300,rulesButtonOn,rulesButtonOff,rulesButtonOver,displayRules,NULL,NULL);
 	button(300,380,startButtonOn,startButtonOff,startButtonOver,restartGame,NULL,NULL);
}
...
var eggsInCarton = 0;					// tracks how many eggs are in the carton. 
var eggsLost = 0;							// tracks how many eggs have hit the ground
var cartonsShipped = 0;					// tracks how many cartons of eggs have been completed
var eggsShipped = 0;						// tracks how mant eggs have been shipped (cartonsShipped * 6) for the final score
